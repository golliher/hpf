...
def officetv():
    env.user = 'pi'
    env.hosts = ['192.168.4.19']
    env.collect_cmd = '/home/pi/photo-frame-project/bin/python /home/pi/photo-frame-project/webremote/manage.py collectstatic --noinput'

def test(): # i.e. vmware on MBP
    env.hosts = ['192.168.4.22']
    collect_cmd = '/Users/dgolliher/.virtualenvs/photoframe/bin/python /home/dgolliher/photo-frame-project/webremote/manage.py collectstatic --noinput'
...
def deploy():
    ...
    print "Copying Static files over"
    put('static','photo-frame-project')
